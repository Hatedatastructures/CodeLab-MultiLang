...
struct format_time
{
    static std::string formatPrint()
    {   // 加8小时为北京时间
        return std::format("[{:%Y-%m-%d %H:%M:%S}]", std::chrono::system_clock::now() + std::chrono::hours(8));
    }

    static std::string formatPrint(const std::string &msg)
    {
        return std::format("[{:%Y-%m-%d %H:%M:%S}] {}", std::chrono::system_clock::now() + std::chrono::hours(8), msg);
    }

    template <typename... strings>
    static std::string formatPrint(const std::string &msg, strings... args)
    {
        return std::format(format_time::formatPrint(msg), args...);
    }
};
 ...
 */
template <typename... strings>
std::string formatPrint(const std::string &msg, strings... args)
{
    return std::vformat(format_time::formatPrint(msg), std::make_format_args(args...));
}
...
struct asset
{
    ...
    asset() = default;
    asset(const std::string &html_path) : file_data(readFile(html_path)) {}
};
...
private:
   ...
   * @param path 文件路径
   */
    std::string mimeType(std::string path)
    {
        auto dot = path.rfind('.');
   ...
   */
    std::string makeAbsolutelyPath(const std::string &file_path)
    {
        if (web_root.empty())
        {
            web_root = "/";
   ...
   */
    http::response<> makeStaticResponse(const std::string &file_path, bool keep_alive)
    {
        ...
        else
        {
            ...
            response.base().set(http::field::content_type, mimeType(file_path));
   ...
   * @return http::response<> 响应
   */
    http::response<> defaultHandleRequest(const http::request<> &request)
    {
        ...
        try
        {
            ...
            if (std::filesystem::exists(full) && std::filesystem::is_regular_file(full))
                return makeStaticResponse(full.string(),keep);
        }
        catch (...)
        {
            return make404Response(false);
        }
   ...
   * @return `http::response<>` 响应
   */
    http::response<> make404Response(bool keep_alive) const
    {
        http::response<> response;
        response.result(boost::beast::http::status::not_found);
        response.base().set(http::field::content_type, "text/html; charset=UTF-8");
   ...
   * @return `http::response<>` 响应
   */
    http::response<> make500Response(bool keep_alive) const
    {
        http::response<> response;
    ...
    static void logSendResult(const std::shared_ptr<session::session<http::request<>, http::response<>>>& sess_ptr,
        const boost::system::error_code& ec)
    {
        if (!ec)
            std::cout << formatPrint("send response success :{}", sess_ptr->get_session_id()) << std::endl;
        else
            std::cout << formatPrint("send response error :{},{}", sess_ptr->get_session_id(), ec.message()) << std::endl;
    }
        ...
        auto handle_function = [&](boost::system::error_code ec, boost::asio::ip::tcp::socket socket)
        {
            if (!ec)
            {
                ...
                auto func = [this](const session_ptr& ptr, std::string_view data)
                {
                    ...
                    {
                        server::logSendResult(sess_ptr, ec);
                    };  // end Lambda call
                    ...
                    {
                        http::response<> bad = make404Response(false);
                        std::cout << formatPrint(" parsing failed ip:{},port:{}",ptr->get_remote_address(),ptr->get_remote_port()) << std::endl;
                    ...
                    try
                    {
                        http::response<> res = defaultHandleRequest(request);
                        std::cout << formatPrint("request success,from ip:{},port:{}",ptr->get_remote_address(),ptr->get_remote_port()) << std::endl;
                        ptr->async_send_response(res, call);
                    }
                    catch (const std::exception &e)
                    {
                        http::response<> err = make500Response(false);
                        std::cout << formatPrint(" server error :{},{}",ptr->get_session_id(),e.what()) << std::endl;
                        auto send_and_close = [sess_ptr = ptr](boost::system::error_code ec)
                        {
                            server::logSendResult(sess_ptr, ec);
                            sess_ptr->close();
                ...
                std::cout << formatPrint("connection successful,from ip {},port:{}",
                ...
                std::cout << formatPrint("{} create session success,id:{} ", value.second->get_remote_address(), 
            ...
            {
                std::cout << formatPrint("accept error:{}", ec.message()) << std::endl;
            }
            if (server_running.load() && acceptor.is_open())
                socketAccept();
...
public:
    ...
    {
        std::cout << formatPrint("{} server initialization succeeded,port:{}", endpoint.address().to_string(), port) << std::endl;
        preloadHtml();
    }
   ...
   * @brief 设置web根目录
   */
    void setWebRoot(const std::string &root)
    {
        web_root = root;
        preloadHtml();
    }
