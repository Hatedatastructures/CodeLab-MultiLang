...
public:
    ...
    // 提供带端口的构造重载，内部调用 bindPort 做安全绑定
     ...
     * @brief 增加上游代理端点
     */
    void increaseEndpoint(std::string domain, std::string host, std::uint16_t port, bool https = false)
    {
     ...
     * @brief 增加异步执行流
     */
    void setExecutionStream(std::shared_ptr<wan::pool::thread_pool> stream)
    {
        thread_pool = stream;
    ...
    void setInsecureSkipVerify(bool enable)
    {
        transponder->set_ssl_insecure_skip_verify(enable);
    }

    void setReceptionProcessing(std::function<void(session_ptr, std::string_view)> processing)
    {
    ...
    void enterMonitoring()
    {
        auto monitoring_function = [this](boost::system::error_code ec, boost::asio::ip::tcp::socket socket)
        {
            if (!ec)
            {
                ...
                {
                    self->forwardProcessing(std::move(*socket_ptr));
                };
    ...
    void forwardProcessing(boost::asio::ip::tcp::socket &&socket)
    {
        auto pair = session_management->create_server_session(std::move(socket));
        if (!pair.second)
            return; // 创建会话失败
        if (reception_processing)
            pair.second->setReceptionProcessing(reception_processing);
        pair.second->start();
    }
};